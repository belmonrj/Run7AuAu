void rat_cent()
{
  ...
  control(pion,neg,0,pion,neg,4); // negpion RCP
  control(pion,pos,0,pion,pos,4); // pospion RCP
  control(kaon,neg,0,kaon,neg,4); // negkaon RCP
  control(kaon,pos,0,kaon,pos,4); // poskaon RCP
  control(prot,neg,0,prot,neg,4); // negprot RCP
  control(prot,pos,0,prot,pos,4); // posprot RCP
...
void step1(int ipidX=0, int ichargeX=0, int centidX=0, int ipidY=0, int ichargeY=0, int centidY=0)
{
  ...
  if(ipidX==0) pidnumberX = 0;
  if(ipidX==1) pidnumberX = 4;
  if(ipidX==2) pidnumberX = 6;
  ...
  if(ipidY==0) pidnumberY = 0;
  if(ipidY==1) pidnumberY = 4;
  if(ipidY==2) pidnumberY = 6;
  ...
  char fnameout[150];
  sprintf(fnameout,"Rout/ratio_reference_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
...
void step2(int ipidX=0, int ichargeX=0, int centidX=0, int ipidY=0, int ichargeY=0, int centidY=0, int itype=0, int iswitch=0)
{
  ...
  //sprintf(fnameinX,"Data/ptpid_tofw_%s_%d_dis_%s.dat",chargestringX,pidnumberX,centstringX);
  //sprintf(fnameinY,"Data/ptpid_tofw_%s_%d_dis_%s.dat",chargestringY,pidnumberY,centstringY);
  sprintf(fnameinX,"Data/ptsys_%s_%s_%s%s_%s.dat",pidstringX,chargestringX,typestring,switchstring,centstringX);
  sprintf(fnameinY,"Data/ptsys_%s_%s_%s%s_%s.dat",pidstringY,chargestringY,typestring,switchstring,centstringY);
  ifstream finX(fnameinX);
  ifstream finY(fnameinY);

  char fnameout[150];
  sprintf(fnameout,"Rout/ratio_sys_%s%s_%s%s%s%s_%d%d_%s%s.dat",typestring,switchstring,chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ofstream fout(fnameout);
...
void step3(int ipidX=0, int ichargeX=0, int centidX=0, int ipidY=0, int ichargeY=0, int centidY=0, int itype=0, int iswitch=0)
{
  ...
  char fnameref[150];
  sprintf(fnameref,"Rout/ratio_reference_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ifstream fref(fnameref);
  char fnamesys[150];
  sprintf(fnamesys,"Rout/ratio_sys_%s%s_%s%s%s%s_%d%d_%s%s.dat",typestring,switchstring,chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ifstream fsys(fnamesys);
  // --
  char fnameout[150];
  sprintf(fnameout,"Rout/uncertain_%s%s_%s%s%s%s_%d%d_%s%s.dat",typestring,switchstring,chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ofstream fout(fnameout);
  ...
  sprintf(figname,"UFig/uncertain_%s%s_%s%s%s%s_%d%d_%s%s.png",typestring,switchstring,chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  c1->Print(figname);
  sprintf(figname,"UFig/uncertain_%s%s_%s%s%s%s_%d%d_%s%s.eps",typestring,switchstring,chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  c1->Print(figname);
  c1->Clear();
  ...
  leg->Draw();
  sprintf(figname,"UFig/compadraw_%s%s_%s%s%s%s_%d%d_%s%s.png",typestring,switchstring,chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  c1->Print(figname);
  sprintf(figname,"UFig/compadraw_%s%s_%s%s%s%s_%d%d_%s%s.eps",typestring,switchstring,chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  c1->Print(figname);
  c1->Clear();
  // delete c1;
  // delete leg;
...
void step4(int ipidX=0, int ichargeX=0, int centidX=0, int ipidY=0, int ichargeY=0, int centidY=0)
{
  ...
  sprintf(fnameinX_1a,"Rout/uncertain_dcfcA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  sprintf(fnameinY_1a,"Rout/uncertain_dcfcA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ifstream finX_1a;
  ifstream finY_1a;
  ...
  sprintf(fnameinX_2a,"Rout/uncertain_dzedA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  sprintf(fnameinY_2a,"Rout/uncertain_dzedA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ifstream finX_2a;
  ifstream finY_2a;
  ...
  sprintf(fnameinX_3a,"Rout/uncertain_pc3mA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  sprintf(fnameinY_3a,"Rout/uncertain_pc3mA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ifstream finX_3a;
  ifstream finY_3a;
  ...
  sprintf(fnameinX_4a,"Rout/uncertain_tfwmA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  sprintf(fnameinY_4a,"Rout/uncertain_tfwmA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ifstream finX_4a;
  ifstream finY_4a;
  ...
  sprintf(fnameinX_5a,"Rout/uncertain_pc1fA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  sprintf(fnameinY_5a,"Rout/uncertain_pc1fA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ifstream finX_5a;
  ifstream finY_5a;
  ...
  sprintf(fnameinX_6a,"Rout/uncertain_pc3fA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  sprintf(fnameinY_6a,"Rout/uncertain_pc3fA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ifstream finX_6a;
  ifstream finY_6a;
  ...
  sprintf(fnameinX_7a,"Rout/uncertain_spidA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  sprintf(fnameinY_7a,"Rout/uncertain_spidA_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ifstream finX_7a;
  ifstream finY_7a;
  ...
  sprintf(fnameinX_1b,"Rout/uncertain_dcfcB_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  sprintf(fnameinY_1b,"Rout/uncertain_dcfcB_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ifstream finX_1b;
  ifstream finY_1b;
  ...
  sprintf(fnameinX_7b,"Rout/uncertain_spidB_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  sprintf(fnameinY_7b,"Rout/uncertain_spidB_%s%s%s%s_%d%d_%s%s.dat",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  ifstream finX_7b;
  ifstream finY_7b;
  ...
  char fignameA[100];
  sprintf(fignameA,"AFig/uncertain_sumA_%s%s%s%s_%d%d_%s%s.png",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  c1->Print(fignameA);
  sprintf(fignameA,"AFig/uncertain_sumA_%s%s%s%s_%d%d_%s%s.eps",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  c1->Print(fignameA);
  ...
  char fignameB[100];
  sprintf(fignameB,"AFig/uncertain_sumB_%s%s%s%s_%d%d_%s%s.png",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  c1->Print(fignameB);
  sprintf(fignameB,"AFig/uncertain_sumB_%s%s%s%s_%d%d_%s%s.eps",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  c1->Print(fignameB);
  ...
  char fignameAa[100];
  sprintf(fignameAa,"AFig/uncertain_sumAa_%s%s%s%s_%d%d_%s%s.png",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  c1->Print(fignameAa);
  sprintf(fignameAa,"AFig/uncertain_sumAa_%s%s%s%s_%d%d_%s%s.eps",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  c1->Print(fignameAa);
  ...
  char fignameBb[100];
  sprintf(fignameBb,"AFig/uncertain_sumBb_%s%s%s%s_%d%d_%s%s.png",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  c1->Print(fignameBb);
  sprintf(fignameBb,"AFig/uncertain_sumBb_%s%s%s%s_%d%d_%s%s.eps",chargestringX,pidstringX,chargestringY,pidstringY,pidnumberX,pidnumberY,centstringX,centstringY);
  c1->Print(fignameBb);
