      ...
      for(int i=0;i<n;i++) // loop over events
	{
	  ...
	  for(int itrk=0; itrk<ktree->mNtrack; itrk++)
	      ...
	      if(regular_cuts&&edge&&inner)
		{
	      ...
	      bool piX = isPiLME>=-1.5 && isPiLME<=1.5 && isKLME<=-2;
	      bool kaX = isKLME>=-1.5 && isKLME<=1.5 && isPiLME>=2 && isPLME<=-2;
	      bool prX = isPLME>=-1.5 && isPLME<=1.5 && isKLME>=2;
	      bool piY = isPiLME>=-2 && isPiLME<=2 && isKLME<=-2.5;
	      bool kaY = isKLME>=-2 && isKLME<=2 && isPiLME>=2.5 && isPLME<=-2.5;
	      bool prY = isPLME>=-2 && isPLME<=2 && isKLME>=2.5;
