void quickispi_sim()
{
  ...
  TFile *file = TFile::Open("simsum_pm.root");
  ...
  for(int i=0; i<20; i++)
    {
      ...
      hhpos->Draw();
      //hhpos->Fit("gaus","","",-2,2);
      hhpos->Fit("fungauspos","","",-2,2);
      sprintf(gifnamepos,"isPi_figs/hhispipos_mom_%d_sim.gif",i);
      sprintf(epsnamepos,"isPi_figs/hhispipos_mom_%d_sim.eps",i);
      c1->Print(gifnamepos);
      c1->Print(epsnamepos);
      ...
      hhneg->Draw();
      //hhneg->Fit("gaus","","",-2,2);
      hhneg->Fit("fungausneg","","",-2,2);
      sprintf(gifnameneg,"isPi_figs/hhispineg_mom_%d_sim.gif",i);
      sprintf(epsnameneg,"isPi_figs/hhispineg_mom_%d_sim.eps",i);
      c1->Print(gifnameneg);
      c1->Print(epsnameneg);
  ...
  tmg_mean->Draw("AP");
  tmg_mean->SetMaximum(0.1);
  tmg_mean->SetMinimum(-2.0);
  ...
  c1->Print("plot_ispi_mean_sim.gif");
  c1->Print("plot_ispi_mean_sim.eps");
  ...
  c1->Print("plot_ispi_sigma_sim.gif");
  c1->Print("plot_ispi_sigma_sim.eps");
