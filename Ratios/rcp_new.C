...
void doit_file(int centid1=0, int centid2=4, int pionid=0, int kaonid=4, int protonid=6)
{
  ...
  finposcb_C.close();
  finnegcb_C.close();
  sprintf(fnameinposcb_C,"FinalFeed/R2_ptpid_tofw_pos_%d_dis_cent%s.dat",pionid,centstring_C);
  sprintf(fnameinnegcb_C,"FinalFeed/R2_ptpid_tofw_neg_%d_dis_cent%s.dat",pionid,centstring_C);
  finposcb_C.open(fnameinposcb_C);
  finnegcb_C.open(fnameinnegcb_C);
  for(int i=0; i<end; i++)
    {
	...
	{
	  finposcb_C>>pT_poscb_C[i+shift]>>yield_poscb_C[i+shift]>>eyield_poscb_C[i+shift];
	  finnegcb_C>>pT_negcb_C[i+shift]>>yield_negcb_C[i+shift]>>eyield_negcb_C[i+shift];
	}
    }
  finposcb_C.close();
  finnegcb_C.close();
  ...
  for(int i=0; i<end3; i++)
      ...
      else
	{
	  ...
	  finposcb_C>>pT_poscb_C[i+shift3]>>yield_poscb_C[i+shift3]>>eyield_poscb_C[i+shift3];
	  finnegcb_C>>pT_negcb_C[i+shift3]>>yield_negcb_C[i+shift3]>>eyield_negcb_C[i+shift3];
	}
    }
  finposcb_C.close();
  finnegcb_C.close();
  ...
  finposcb_P.close();
  finnegcb_P.close();
  sprintf(fnameinposcb_P,"FinalFeed/R2_ptpid_tofw_pos_%d_dis_cent%s.dat",pionid,centstring_P);
  sprintf(fnameinnegcb_P,"FinalFeed/R2_ptpid_tofw_neg_%d_dis_cent%s.dat",pionid,centstring_P);
  finposcb_P.open(fnameinposcb_P);
  finnegcb_P.open(fnameinnegcb_P);
  for(int i=0; i<end; i++)
    {
	...
	{
	  finposcb_P>>pT_poscb_P[i+shift]>>yield_poscb_P[i+shift]>>eyield_poscb_P[i+shift];
	  finnegcb_P>>pT_negcb_P[i+shift]>>yield_negcb_P[i+shift]>>eyield_negcb_P[i+shift];
	}
    }
  finposcb_P.close();
  finnegcb_P.close();
  ...
  for(int i=0; i<end3; i++)
	...
	{
	  if(i+shift3>40) break;
	  finposcb_P>>pT_poscb_P[i+shift3]>>yield_poscb_P[i+shift3]>>eyield_poscb_P[i+shift3];
	  finnegcb_P>>pT_negcb_P[i+shift3]>>yield_negcb_P[i+shift3]>>eyield_negcb_P[i+shift3];
	}
    }
  finposcb_P.close();
  finnegcb_P.close();
  ...
  finposcb_C.close();
  finnegcb_C.close();
  sprintf(fnameinposcb_C,"FinalFeed/R2_ptpid_tofw_pos_%d_dis_cent%s.dat",kaonid,centstring_C);
  sprintf(fnameinnegcb_C,"FinalFeed/R2_ptpid_tofw_neg_%d_dis_cent%s.dat",kaonid,centstring_C);
  finposcb_C.open(fnameinposcb_C);
  finnegcb_C.open(fnameinnegcb_C);
  for(int i=0; i<end; i++)
    {
  ...
  finposcb_P.close();
  finnegcb_P.close();
  sprintf(fnameinposcb_P,"FinalFeed/R2_ptpid_tofw_pos_%d_dis_cent%s.dat",kaonid,centstring_P);
  sprintf(fnameinnegcb_P,"FinalFeed/R2_ptpid_tofw_neg_%d_dis_cent%s.dat",kaonid,centstring_P);
  finposcb_P.open(fnameinposcb_P);
  finnegcb_P.open(fnameinnegcb_P);
  for(int i=0; i<end; i++)
    {
  ...
  finposcb_C.close();
  finnegcb_C.close();
  sprintf(fnameinposcb_C,"FinalFeed/R2_ptpid_tofw_pos_%d_dis_cent%s.dat",protonid,centstring_C);
  sprintf(fnameinnegcb_C,"FinalFeed/R2_ptpid_tofw_neg_%d_dis_cent%s.dat",protonid,centstring_C);
  finposcb_C.open(fnameinposcb_C);
  finnegcb_C.open(fnameinnegcb_C);
  for(int i=0; i<end; i++)
    {
  ...
  finposcb_P.close();
  finnegcb_P.close();
  sprintf(fnameinposcb_P,"FinalFeed/R2_ptpid_tofw_pos_%d_dis_cent%s.dat",protonid,centstring_P);
  sprintf(fnameinnegcb_P,"FinalFeed/R2_ptpid_tofw_neg_%d_dis_cent%s.dat",protonid,centstring_P);
  finposcb_P.open(fnameinposcb_P);
  finnegcb_P.open(fnameinnegcb_P);
  for(int i=0; i<end; i++)
    {
