...
void doit_file(int centid=0, int pionid=0, int protonid=6)
{
  ...
  //for(int i=0; i<nbin; i++) ept[i]=sqrt((0.01*pt[i])**2+(0.01*(pt[i]*pt[i]))**2);  // yeah
  ...
  TGraphErrors *tge_syspos = new TGraphErrors(nbin,pt,ratiopos,0,esysratiopos);
  tge_syspos->SetMarkerColor(kRed);
  tge_syspos->SetMarkerStyle(1);
  tge_syspos->SetLineColor(kRed-10);
  tge_syspos->SetLineWidth(15);
  TGraphErrors *tge_sysneg = new TGraphErrors(nbin,pt,rationeg,0,esysrationeg);
