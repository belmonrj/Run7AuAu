      ...
      for(int i=0;i<n;i++) // loop over events
	{
	  ...
	  for(int itrk=0; itrk<ktree->mNtrack; itrk++)
	      ...
	      if(striptofw<256)
		{
	      ...
	      if(zed>0) hh_momphi_poszed_1->Fill(phi,alpha);
	      if(zed<0) hh_momphi_negzed_1->Fill(phi,alpha);
	      ...
	      if(zed<0)
		{
	      ...
	      if(zed>0) hh_momphi_poszed_2->Fill(phi,alpha);
	      if(zed<0) hh_momphi_negzed_2->Fill(phi,alpha);
	      ...
	      if(zed>0) hh_momphi_poszed_3->Fill(phi,alpha);
	      if(zed<0) hh_momphi_negzed_3->Fill(phi,alpha);
	      ...
	      if(zed>0) hh_momphi_poszed_4->Fill(phi,alpha);
	      if(zed<0) hh_momphi_negzed_4->Fill(phi,alpha);
	      ...
	      if(zed>0) hh_momphi_poszed_5->Fill(phi,alpha);
	      if(zed<0) hh_momphi_negzed_5->Fill(phi,alpha);
	      ...
	      if(zed>0) hh_momphi_poszed_6->Fill(phi,alpha);
	      if(zed<0) hh_momphi_negzed_6->Fill(phi,alpha);
	      ...
	      if(zed>0) hh_momphi_poszed_7->Fill(phi,alpha);
	      if(zed<0) hh_momphi_negzed_7->Fill(phi,alpha);
	      ...
	      else if(run>=236133&&run<=240121)
		{
	      ...
	      if(zed>0) hh_momphi_poszed_8->Fill(phi,alpha);
	      if(zed<0) hh_momphi_negzed_8->Fill(phi,alpha);
  ...
  if(zed>0)
    {
      ...
      cutg->SetPoint(0,-0.015,0.000);
      cutg->SetPoint(1,0.024,0.000);
      cutg->SetPoint(2,0.024,0.070);
      cutg->SetPoint(3,-0.015,0.000);
      if(cutg->IsInside(phi,alpha)) return false;
      TCutG *cutg2 = new TCutG("CUTG2",4); // assoc. w/ stripe 2
      cutg2->SetPoint(0,-0.058,0.000);
      cutg2->SetPoint(1,-0.020,0.000);
      cutg2->SetPoint(2,-0.058,-0.068);
      cutg2->SetPoint(3,-0.058,0.000);
      //if(cutg2->IsInside(phi,alpha)) return false;
      TCutG *cutg3 = new TCutG("CUTG3",4); // assoc. w/ stripe 3
      cutg3->SetPoint(0,-0.040,0.000);
      cutg3->SetPoint(1,-0.020,0.000);
      cutg3->SetPoint(2,-0.040,-0.047);
      cutg3->SetPoint(3,-0.040,0.000);
      if(cutg3->IsInside(phi,alpha)) return false;
    }
  if(zed<0)
    {
      if(fabs(alpha)<0.002) return false; // false high momentum tracks
