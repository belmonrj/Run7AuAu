...
{ 
  ...
  // no cuts
  // zed>0
  //TH2F *hh_alphaphi_poszed_nc = new TH2F("hh_alphaphi_poszed_nc","alpha vs phi zed>0 no cuts", 400, -0.2, 0.2, 200, -0.1, 0.1); // ORIGINAL. SAVE!
      ...
      for(int i=0;i<n;i++) // loop over events
	{
	  ...
	  for(int itrk=0; itrk<ktree->mNtrack; itrk++)
	      ...
	      if(striptofw<256)
		{
	      ...
	      if(charge>0)
		{
		  if(fabs(pc3sdphi)<2.0)  pc3dz_dis_pos->Fill(mom,pc3sdz);
		  if(fabs(pc3sdz)<2.0)    pc3dphi_dis_pos->Fill(mom,pc3sdphi);
		  if(fabs(tofwsdphi)<2.0) tofwdz_dis_pos->Fill(mom,tofwsdz);
		  if(fabs(tofwsdz)<2.0)   tofwdphi_dis_pos->Fill(mom,tofwsdphi);
		}
	      // neg
	      if(charge<0)
		{
		  if(fabs(pc3sdphi)<2.0)  pc3dz_dis_neg->Fill(mom,pc3sdz);
		  if(fabs(pc3sdz)<2.0)    pc3dphi_dis_neg->Fill(mom,pc3sdphi);
		  if(fabs(tofwsdphi)<2.0) tofwdz_dis_neg->Fill(mom,tofwsdz);
		  if(fabs(tofwsdz)<2.0)   tofwdphi_dis_neg->Fill(mom,tofwsdphi);
		}
	      */
	      // ----------------------- //
	      // --- test histograms --- //
  ...
  if(zed>0)
    {
      ...
      if(phi>-0.099&&phi<-0.089) return false;
      if(phi>-0.058&&phi<-0.052) return false;
      if(phi>-0.039&&phi<-0.032) return false;
      if(phi>-0.021&&phi<-0.013) return false; // original
      //if(phi>-0.024&&phi<-0.01) return false; // modified
      if(phi>0.020&&phi<0.026) return false;
      if(phi>0.039&&phi<0.046) return false; // original
      //if(phi>0.034&&phi<0.047) return false; // modified
      if(phi>0.056&&phi<0.065) return false;
      if(phi>0.077&&phi<0.085) return false; // original
      //if(phi>0.076&&phi<0.085) return false; // modified
      // === W2 === //
      if(alpha<1.778*phi-1.778*0.3   && alpha>1.778*phi-1.778*0.418) return false; // w2 below box
      if(alpha<1.778*phi-1.778*0.494 && alpha>1.778*phi-1.778*0.501) return false; // w2 center of box
      if(alpha<1.778*phi-1.778*0.576 && alpha>1.778*phi-1.778*0.7  ) return false; // w2 above box
      //if(phi>0.390&&phi<0.393) return false;
      if(phi>0.410&&phi<0.419) return false;
      if(phi>0.430&&phi<0.438) return false;
      if(phi>0.469&&phi<0.475) return false;
      //if(phi>0.488&&phi<0.491) return false;
      if(phi>0.509&&phi<0.516) return false;
      if(phi>0.526&&phi<0.533) return false;
      //if(phi>0.537&&phi<0.541) return false;
      //if(phi>0.547&&phi<0.555) return false;
      if(phi>0.563&&phi<0.573) return false;
      if(phi>0.585&&phi<0.592) return false;
      //if(phi>0.601&&phi<0.603) return false;
    }
  if(zed<0)
    {
      if(fabs(alpha)<0.002) return false; // false high momentum tracks
      ...
      if(phi>-0.115&&phi<-0.089) return false; // wider than poszed
      if(phi>-0.058&&phi<-0.052) return false;
      if(phi>-0.039&&phi<-0.032) return false;
      if(phi>-0.021&&phi<-0.013) return false; // original
      //if(phi>-0.024&&phi<-0.01) return false; // modified
      ...
      //if(phi>0.034&&phi<0.047) return false; // wider than poszed
      if(phi>0.056&&phi<0.065) return false;
      ...
      if(phi>0.430&&phi<0.438) return false;
      if(phi>0.469&&phi<0.475) return false;
      if(phi>0.488&&phi<0.494) return false; // commented out for W1, made wider here
      if(phi>0.509&&phi<0.516) return false;
      if(phi>0.526&&phi<0.533) return false;
      //if(phi>0.537&&phi<0.541) return false;
      //if(phi>0.547&&phi<0.555) return false;
