      ...
      for(int i=0;i<n;i++) // loop over events
	{
	  ...
	  for(int itrk=0; itrk<ktree->mNtrack; itrk++)
	      ...
	      if(zed<0)
		{
	      ...
	      bool dis1 = isPiLow>=-2 && isPiLow<=2 && n0<0;
...
bool GoodInnerDCH(int run, float zed, float alpha, float phi)
{
  ...
  if(alpha>22.3703*phi-22.3703*0.1205 && alpha>-21.3095*phi+21.3095*0.0383544 && alpha>0) return false; // checked, preferred for now
  ...
  if(phi>0.185 && phi<0.225 && alpha<0) return false;
  ...
  if(zed>0)
    {
      ...
      // left-most corner
      //if(phi<-0.373) return false; // not needed for Run7
      // small stripe, lower alpha, lower phi
      if(alpha>23.7666*phi-23.7666*-0.225933 && alpha<14.5819*phi-14.5819*-0.227896) return false; // checked for Run7 (almost perfect)
      ...
      if(alpha<25.1495*phi-25.1495*0.178771 && alpha>12.7551*phi-12.7551*0.195642) return false;
  ...
  if(zed<0)
    {
      ...
      // --- triangular hole for lower alpha, lower phi, see next 6 lines
      //if(alpha<-17.3768*phi+17.3768*-0.191001 && alpha<18.8489*phi-18.8489*-0.200406) return false; // checked, see next 5 lines
      ...
      if(run>235809 && alpha<18.8489*phi-18.8489*-0.202 && alpha>22.1084*phi-22.1084*-0.193) return false; // b // checked
      if(run<235810 && alpha<18.8489*phi-18.8489*-0.205 && alpha>22.1084*phi-22.1084*-0.193) return false; // b // checked
      
      // --- parellel hole with negative slope for upper alpha, higher phi
      if(alpha<-14.8469*phi+14.8469*0.199158 && alpha>-13.1329*phi+13.1329*0.181353 && alpha>0) return false; // checked

      // slice, upper alpha, high phi
      if(alpha<25.1495*phi-25.1495*0.178771 && alpha>12.7551*phi-12.7551*0.201) return false; // wider than North side

      // === W2 === //
      // --- small stripe, upper alpha, higher phi
      //if(alpha<22.0*phi-22.0*0.725 && alpha>22.0*phi-22.0*0.745) return false; // checked // run8 only
