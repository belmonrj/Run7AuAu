...
float isPionLowMean(float x, float m2tofw);
float isKaonLowMean(float x, float m2tofw);
float isProtonLowMean(float x, float m2tofw);
      ...
      for(int i=0;i<n;i++) // loop over events
	{
	  ...
	  for(int itrk=0; itrk<ktree->mNtrack; itrk++)
	      ...
	      if(regular_cuts&&edge&&inner&&zed<0)
		{
	      ...
	      float isPiLowMean = isPionLowMean(pT,m2tofw);
	      float isKLowMean  = isKaonLowMean(pT,m2tofw);
	      float isPLowMean  = isProtonLowMean(pT,m2tofw);

	      //isPiLowMean /= 0.85; // SUPER TEMP
	      //isPiANA /= 0.85; // SUPER TEMP

	      bool isPi = (isPiLowMean>=-2&&isPiLowMean<=2);
	      bool isK = (isKLowMean>=-2&&isKLowMean<=2);
	      bool isP = (isPLowMean>=-2&&isPLowMean<=2);
	      ...
	      if(regular_cuts&&edge&&inner)
		{
		  ...
		  if(cent> 0&&cent<=10&&charge>0) isPi_dis_pos_cent0010->Fill(pT,isPiLowMean);//important
		  if(cent> 0&&cent<=10&&charge<0) isPi_dis_neg_cent0010->Fill(pT,isPiLowMean);//important
		  if(cent>10&&cent<=20&&charge>0) isPi_dis_pos_cent1020->Fill(pT,isPiLowMean);//important
		  if(cent>10&&cent<=20&&charge<0) isPi_dis_neg_cent1020->Fill(pT,isPiLowMean);//important
		  if(cent>20&&cent<=40&&charge>0) isPi_dis_pos_cent2040->Fill(pT,isPiLowMean);//important
		  if(cent>20&&cent<=40&&charge<0) isPi_dis_neg_cent2040->Fill(pT,isPiLowMean);//important
		  if(cent>40&&cent<=60&&charge>0) isPi_dis_pos_cent4060->Fill(pT,isPiLowMean);//important
		  if(cent>40&&cent<=60&&charge<0) isPi_dis_neg_cent4060->Fill(pT,isPiLowMean);//important
		  if(cent>60&&cent<=92&&charge>0) isPi_dis_pos_cent6092->Fill(pT,isPiLowMean);//important
		  if(cent>60&&cent<=92&&charge<0) isPi_dis_neg_cent6092->Fill(pT,isPiLowMean);//important
		  if(cent> 0&&cent<=10&&charge>0) isK_dis_pos_cent0010->Fill(pT,isKLowMean);//important
		  if(cent> 0&&cent<=10&&charge<0) isK_dis_neg_cent0010->Fill(pT,isKLowMean);//important
		  if(cent>10&&cent<=20&&charge>0) isK_dis_pos_cent1020->Fill(pT,isKLowMean);//important
		  if(cent>10&&cent<=20&&charge<0) isK_dis_neg_cent1020->Fill(pT,isKLowMean);//important
		  if(cent>20&&cent<=40&&charge>0) isK_dis_pos_cent2040->Fill(pT,isKLowMean);//important
		  if(cent>20&&cent<=40&&charge<0) isK_dis_neg_cent2040->Fill(pT,isKLowMean);//important
		  if(cent>40&&cent<=60&&charge>0) isK_dis_pos_cent4060->Fill(pT,isKLowMean);//important
		  if(cent>40&&cent<=60&&charge<0) isK_dis_neg_cent4060->Fill(pT,isKLowMean);//important
		  if(cent>60&&cent<=92&&charge>0) isK_dis_pos_cent6092->Fill(pT,isKLowMean);//important
		  if(cent>60&&cent<=92&&charge<0) isK_dis_neg_cent6092->Fill(pT,isKLowMean);//important
		  if(cent> 0&&cent<=10&&charge>0) isP_dis_pos_cent0010->Fill(pT,isPLowMean);//important
		  if(cent> 0&&cent<=10&&charge<0) isP_dis_neg_cent0010->Fill(pT,isPLowMean);//important
		  if(cent>10&&cent<=20&&charge>0) isP_dis_pos_cent1020->Fill(pT,isPLowMean);//important
		  if(cent>10&&cent<=20&&charge<0) isP_dis_neg_cent1020->Fill(pT,isPLowMean);//important
		  if(cent>20&&cent<=40&&charge>0) isP_dis_pos_cent2040->Fill(pT,isPLowMean);//important
		  if(cent>20&&cent<=40&&charge<0) isP_dis_neg_cent2040->Fill(pT,isPLowMean);//important
		  if(cent>40&&cent<=60&&charge>0) isP_dis_pos_cent4060->Fill(pT,isPLowMean);//important
		  if(cent>40&&cent<=60&&charge<0) isP_dis_neg_cent4060->Fill(pT,isPLowMean);//important
		  if(cent>60&&cent<=92&&charge>0) isP_dis_pos_cent6092->Fill(pT,isPLowMean);//important
		  if(cent>60&&cent<=92&&charge<0) isP_dis_neg_cent6092->Fill(pT,isPLowMean);//important
		  if(charge>0) isPi_dis_pos->Fill(pT,isPiLowMean);//important
		  if(charge<0) isPi_dis_neg->Fill(pT,isPiLowMean);//important
		  if(charge>0) isK_dis_pos->Fill(pT,isKLowMean);
		  if(charge<0) isK_dis_neg->Fill(pT,isKLowMean);
		  if(charge>0) isP_dis_pos->Fill(pT,isPLowMean);
		  if(charge<0) isP_dis_neg->Fill(pT,isPLowMean);
	      ...
	      // // pions
	      // bool dis0 = isPiLowMean>=-2 && isPiLowMean<=2 && isKLowMean<=-2 && good_emc;
	      // bool dis1 = isPiANA>=-2 && isPiANA<=2 && isKANA<=-2 && good_emc;
	      // bool dis2 = dis0 && ep>0.2;
	      // bool dis3 = dis1 && ep>0.2;
	      // //kaons
	      // bool dis4 = isKLowMean>=-2 && isKLowMean<=2 && isPiLowMean>=2 && isPLowMean<=-2;
	      // bool dis5 = isKANA>=-2 && isKANA<=2 && isPiANA>=2 && isPANA<=-2;
	      // // protons
	      // bool dis6 = isPLowMean>=-2 && isPLowMean<=2 && isKLowMean>=2 && good_emc;
	      ...
	      // pions
	      bool dis0 = isPiLowMean>=-2 && isPiLowMean<=2 && isKLowMean<=-2 && good_emc;
	      bool dis1 = isPiLow>=-2 && isPiLow<=2 && isKLow<=-2 && good_emc;
	      bool dis2 = dis0 && ep>0.2;
	      bool dis3 = dis1 && ep>0.2;
	      //kaons
	      bool dis4 = isKLowMean>=-2 && isKLowMean<=2 && isPiLowMean>=2 && isPLowMean<=-2;
	      ...
	      // protons
	      bool dis6 = isPLowMean>=-2 && isPLowMean<=2 && isKLowMean>=2 && good_emc;
	      bool dis7 = isPLow>=-2 && isPLow<=2 && isKLow>=2 && good_emc;
	      bool dis8 = dis6 && ep>0.2;
	      bool dis9 = dis7 && ep>0.2;
	      ...
	      if(regular_cuts&&edge&&inner) // ok
		{
	      ...
	      bool piA = isPiLowMean>=-1.5 && isPiLowMean<=1.5 && isKLowMean<=-2;
	      bool kaA = isKLowMean>=-1.5 && isKLowMean<=1.5 && isPiLowMean>=2 && isPLowMean<=-2;
	      bool prA = isPLowMean>=-1.5 && isPLowMean<=1.5 && isKLowMean>=2;
	      bool piB = isPiLowMean>=-2 && isPiLowMean<=2 && isKLowMean<=-2.5;
	      bool kaB = isKLowMean>=-2 && isKLowMean<=2 && isPiLowMean>=2.5 && isPLowMean<=-2.5;
	      bool prB = isPLowMean>=-2 && isPLowMean<=2 && isKLowMean>=2.5;

	      // bool piA = isPiLowMean>=-3 && isPiLowMean<=3 && isKLowMean<=-3;
	      // bool kaA = isKLowMean>=-3 && isKLowMean<=3 && isPiLowMean>=3 && isPLowMean<=-3;
	      // bool prA = isPLowMean>=-3 && isPLowMean<=3 && isKLowMean>=3;
	      // bool piB = isPiLowMean>=-1 && isPiLowMean<=1 && isKLowMean<=-1;
	      // bool kaB = isKLowMean>=-1 && isKLowMean<=1 && isPiLowMean>=1 && isPLowMean<=-1;
	      // bool prB = isPLowMean>=-1 && isPLowMean<=1 && isKLowMean>=1;
