  ...
  if(zed>0)
    {
      ...
      if(alpha>0.000&&phi<0.024&&alpha<1.778*phi-1.778*-0.015) return false; // triangle assoc w/ stripe 5
    }
  if(zed<0)
    {
      if(fabs(alpha)<0.002) return false; // false high momentum tracks
      ...
      if(phi>0.075&&phi<0.081) return false; // 8
      //cutg->SetPoint(0,0.01436782,0.05137712);
      //cutg->SetPoint(1,0.01436782,0.002648305);
      //cutg->SetPoint(2,-0.01364943,0.002648305);
      //cutg->SetPoint(3,0.01364943,0.05243644);
      //cutg->SetPoint(4,0.01436782,0.05137712);
      if(alpha<0.000&&phi>-0.033&&alpha>1.778*phi-1.778*-0.015) return false; // triangle assoc w/ stripe 3
      if(alpha>0.000&&phi<0.024&&alpha<1.778*phi-1.778*-0.015) return false; // triangle assoc w/ stripe 5
    }
