...
//#include "/phenix/u/workarea/belmonrj/offline/AnalysisTrain/HadronRun8/HadronEventRun8.h"
//#include "/phenix/u/workarea/belmonrj/offline/AnalysisTrain/HadronRun8/HadronTrackRun8.h"
#include "/phenix/plhf/belmonrj/tmp/Ron_01_09/HadronRun8/HadronEventRun8.h"
#include "/phenix/plhf/belmonrj/tmp/Ron_01_09/HadronRun8/HadronTrackRun8.h"
...
//   TH2F *hh_alphaphi_negzed_ac_EAST = new TH2F("hh_alphaphi_negzed_ac_EAST","alpha vs phi zed<0 all cuts", 1500, 2.5, 4.0, 1200, -0.6, 0.6);
  ...
  TH2F *isPi_dis_pos_cent0020 = new TH2F("isPi_dis_pos_cent0020","isPi vs mom", 100, 0.0, 10.0, 100, -3.0, 7.0);
  TH2F *isPi_dis_neg_cent0020 = new TH2F("isPi_dis_neg_cent0020","isPi vs mom", 100, 0.0, 10.0, 100, -3.0, 7.0);
  ...
  TH2F *isPi_dis_pos_cent6088 = new TH2F("isPi_dis_pos_cent6088","isPi vs mom", 100, 0.0, 10.0, 100, -3.0, 7.0);
  TH2F *isPi_dis_neg_cent6088 = new TH2F("isPi_dis_neg_cent6088","isPi vs mom", 100, 0.0, 10.0, 100, -3.0, 7.0);

  TH2F *isK_dis_pos_cent0020 = new TH2F("isK_dis_pos_cent0020","isK vs mom", 100, 0.0, 10.0, 100, -5.0, 5.0);
  TH2F *isK_dis_neg_cent0020 = new TH2F("isK_dis_neg_cent0020","isK vs mom", 100, 0.0, 10.0, 100, -5.0, 5.0);
  TH2F *isK_dis_pos_cent2040 = new TH2F("isK_dis_pos_cent2040","isK vs mom", 100, 0.0, 10.0, 100, -5.0, 5.0);
  TH2F *isK_dis_neg_cent2040 = new TH2F("isK_dis_neg_cent2040","isK vs mom", 100, 0.0, 10.0, 100, -5.0, 5.0);
  TH2F *isK_dis_pos_cent4060 = new TH2F("isK_dis_pos_cent4060","isK vs mom", 100, 0.0, 10.0, 100, -5.0, 5.0);
  TH2F *isK_dis_neg_cent4060 = new TH2F("isK_dis_neg_cent4060","isK vs mom", 100, 0.0, 10.0, 100, -5.0, 5.0);
  TH2F *isK_dis_pos_cent6088 = new TH2F("isK_dis_pos_cent6088","isK vs mom", 100, 0.0, 10.0, 100, -5.0, 5.0);
  TH2F *isK_dis_neg_cent6088 = new TH2F("isK_dis_neg_cent6088","isK vs mom", 100, 0.0, 10.0, 100, -5.0, 5.0);

  TH2F *isP_dis_pos_cent0020 = new TH2F("isP_dis_pos_cent0020","isP vs mom", 100, 0.0, 10.0, 100, -7.0, 3.0);
  TH2F *isP_dis_neg_cent0020 = new TH2F("isP_dis_neg_cent0020","isP vs mom", 100, 0.0, 10.0, 100, -7.0, 3.0);
  TH2F *isP_dis_pos_cent2040 = new TH2F("isP_dis_pos_cent2040","isP vs mom", 100, 0.0, 10.0, 100, -7.0, 3.0);
  TH2F *isP_dis_neg_cent2040 = new TH2F("isP_dis_neg_cent2040","isP vs mom", 100, 0.0, 10.0, 100, -7.0, 3.0);
  TH2F *isP_dis_pos_cent4060 = new TH2F("isP_dis_pos_cent4060","isP vs mom", 100, 0.0, 10.0, 100, -7.0, 3.0);
  TH2F *isP_dis_neg_cent4060 = new TH2F("isP_dis_neg_cent4060","isP vs mom", 100, 0.0, 10.0, 100, -7.0, 3.0);
  TH2F *isP_dis_pos_cent6088 = new TH2F("isP_dis_pos_cent6088","isP vs mom", 100, 0.0, 10.0, 100, -7.0, 3.0);
  TH2F *isP_dis_neg_cent6088 = new TH2F("isP_dis_neg_cent6088","isP vs mom", 100, 0.0, 10.0, 100, -7.0, 3.0);
  ...
  while(fin.getline(filename,100))
      ...
      if(!t)
	{
      ...
      HadronEventRun8 *event = new HadronEventRun8();
      TBranch *b = t->GetBranch("pHadronEventRun8");
      b->SetAddress(&event);
      ...
      for(int i=0;i<n;i++) // loop over events
	{
	  //ktree->GetEntry(i); 
	  b->GetEntry(i); 
	  TClonesArray *tracks = event->GetHadronTracks();
	  ...
	  //float bbcz = event->bbcz;
	  float cent = event->cent;
	  ...
	  for(int itrk=0; itrk<event->mNtrack; itrk++)
	    {

	      HadronTrackRun8 *track = dynamic_cast<HadronTrackRun8 *>((*tracks)[itrk]);
	      ...
	      if(charge<0)
		{
	      ...
	      //int dcarm       = ktree->HadronTracks_dcarm[itrk];//old way
	      //int dcarm       = track->get_dcarm();//new way
	      //int n0          = track->get_n0();
	      //int charge      = track->get_charge();
	      float alpha     = track->get_alpha();
	      float phi       = track->get_phi();
	      //float phi0      = track->get_phi0();
	      float the0      = track->get_the0();
	      float mom       = track->get_mom();
	      float zed       = track->get_zed();
	      float pc3sdz    = track->get_pc3sdz();
	      float pc3sdphi  = track->get_pc3sdphi();

	      float tofwdz    = track->get_tofwdz();
	      float tofwdphi  = track->get_tofwdphi();
	      int striptofw   = (int)track->get_striptofw();
	      float tofwadcup = track->get_tofwadcup();
	      float tofwadcdw = track->get_tofwadcdw();
	      float pltofw    = track->get_pltofw();
	      float ttofw     = track->get_ttofw();

	      //float tofedz    = track->get_tofedz();
	      //float tofedphi  = track->get_tofedphi();
	      //float tofesdz    = track->get_tofesdz();
	      //float tofesdphi  = track->get_tofesdphi();
	      float tofesdz    = track->get_tofsdz();
	      float tofesdphi  = track->get_tofsdphi();
	      float etof      = track->get_etof();
	      int slat        = (int)track->get_slat();
	      //float pltofe    = track->get_pltofe();
	      //float ttofe     = track->get_ttofe();
	      float pltofe    = track->get_pltof();
	      float ttofe     = track->get_ttof();

	      //float ecent     = track->get_ecent();
	      float ecore     = track->get_ecore();
	      ...
	      if(pT<1.0)
		{
	      ...
	      bool tfe_matching = fabs(tofesdz)<2.0 && fabs(tofesdphi)<2.0;
	      float sigma_tofw = sqrt(tofwsdz*tofwsdz+tofwsdphi*tofwsdphi);// NEW!!!
	      //bool pc3_matching = fabs(pc3sdz)<3.0 && fabs(pc3sdphi)<3.0;
	      //bool tfw_matching = fabs(tofwsdz)<3.0 && fabs(tofwsdphi)<3.0;
	      //bool tfe_matching = fabs(tofesdz)<3.0 && fabs(tofesdphi)<3.0;
	      ...
	      if(westarm)
		{
	      ...
	      float ppc1x   = track->get_ppc1x();
	      float ppc1y   = track->get_ppc1y();
	      float ppc1z   = track->get_ppc1z();
	      float pc1z    = ppc1z;
	      ...
	      float ppc3x   = track->get_ppc3x();
	      float ppc3y   = track->get_ppc3y();
	      float ppc3z   = track->get_ppc3z();
	      float pc3dz   = track->get_pc3dz();
	      float pc3dphi = track->get_pc3dphi();
	      ...
	      if(charge>0)
		{
		  ...
		  if(westarm&&ecore/pT>0.2) m2pTdis_epc_pos->Fill(pT,m2tofw);
	      ...
	      else
		{
		  ...
		  if(westarm&&ecore/pT>0.2) m2pTdis_epc_neg->Fill(pT,m2tofw);
	      ...
	      //if(pT<1.0) pc3_matching = true; // NEW!!!  temp?
	      ...
	      if(charge<0)
		{
	      ...
	      if(regular_cuts&&edge&&inner)
		{
		  if(cent>00&&cent<=20&&charge>0) isPi_dis_pos_cent0020->Fill(pT,isPi);//important
		  if(cent>00&&cent<=20&&charge<0) isPi_dis_neg_cent0020->Fill(pT,isPi);//important
		  if(cent>20&&cent<=40&&charge>0) isPi_dis_pos_cent2040->Fill(pT,isPi);//important
		  if(cent>20&&cent<=40&&charge<0) isPi_dis_neg_cent2040->Fill(pT,isPi);//important
		  if(cent>40&&cent<=60&&charge>0) isPi_dis_pos_cent4060->Fill(pT,isPi);//important
		  if(cent>40&&cent<=60&&charge<0) isPi_dis_neg_cent4060->Fill(pT,isPi);//important
		  if(cent>60&&cent<=88&&charge>0) isPi_dis_pos_cent6088->Fill(pT,isPi);//important
		  if(cent>60&&cent<=88&&charge<0) isPi_dis_neg_cent6088->Fill(pT,isPi);//important
		  if(cent>00&&cent<=20&&charge>0) isK_dis_pos_cent0020->Fill(pT,isK);//important
		  if(cent>00&&cent<=20&&charge<0) isK_dis_neg_cent0020->Fill(pT,isK);//important
		  if(cent>20&&cent<=40&&charge>0) isK_dis_pos_cent2040->Fill(pT,isK);//important
		  if(cent>20&&cent<=40&&charge<0) isK_dis_neg_cent2040->Fill(pT,isK);//important
		  if(cent>40&&cent<=60&&charge>0) isK_dis_pos_cent4060->Fill(pT,isK);//important
		  if(cent>40&&cent<=60&&charge<0) isK_dis_neg_cent4060->Fill(pT,isK);//important
		  if(cent>60&&cent<=88&&charge>0) isK_dis_pos_cent6088->Fill(pT,isK);//important
		  if(cent>60&&cent<=88&&charge<0) isK_dis_neg_cent6088->Fill(pT,isK);//important
		  if(cent>00&&cent<=20&&charge>0) isP_dis_pos_cent0020->Fill(pT,isP);//important
		  if(cent>00&&cent<=20&&charge<0) isP_dis_neg_cent0020->Fill(pT,isP);//important
		  if(cent>20&&cent<=40&&charge>0) isP_dis_pos_cent2040->Fill(pT,isP);//important
		  if(cent>20&&cent<=40&&charge<0) isP_dis_neg_cent2040->Fill(pT,isP);//important
		  if(cent>40&&cent<=60&&charge>0) isP_dis_pos_cent4060->Fill(pT,isP);//important
		  if(cent>40&&cent<=60&&charge<0) isP_dis_neg_cent4060->Fill(pT,isP);//important
		  if(cent>60&&cent<=88&&charge>0) isP_dis_pos_cent6088->Fill(pT,isP);//important
		  if(cent>60&&cent<=88&&charge<0) isP_dis_neg_cent6088->Fill(pT,isP);//important
	      ...
	      bool pion1 = westarm && fabs(isPi)<2.0;
	      bool pion = dis1;

	      /*
	      if(westarm&&charge>0&&sigma_tofw<10)
		{
...
//    1  p0           5.99661e-02   1.51179e-05   4.76920e-08   3.55699e+00
//    2  p1          -4.08485e-02   8.32154e-06   2.82048e-08  -2.53810e+00
//    3  p2           6.99457e-03   3.07411e-06   1.58511e-08  -6.32859e+00
//    4  p3           8.21363e-02   6.35201e-06   3.91657e-08   1.64187e+01
//    5  p4          -2.34551e-01   1.72924e-05   1.11843e-07   6.24732e+00
// ****************************************
...
// Edm                       =  3.53865e-06
// NCalls                    =           91
// p0                        =   0.00465694   +/-   0.00788637  
// p1                        =   -0.0474435   +/-   0.00743173  
// p2                        =    0.0180152   +/-   0.00327102  
// p3                        =    0.0701986   +/-   0.00190766  
// p4                        =    -0.148939   +/-   0.00645626  
// ****************************************
// Minimizer is Minuit / Migrad
// Chi2                      =  0.000122061
// NDf                       =           20
// Edm                       =  7.23946e-08
// NCalls                    =          146
// p0                        =   -0.0505685   +/-   0.00571118  
// p1                        =    0.0231001   +/-   0.00564648  
// p2                        =   -0.0020908   +/-   0.00249257  
// p3                        =    0.0777132   +/-   0.00127642  
// p4                        =    -0.144319   +/-   0.00454626  
